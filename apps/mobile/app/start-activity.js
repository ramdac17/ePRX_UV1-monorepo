...
export default function StartActivity() {
    ...
    const startTracking = async () => {
        locationSubscription.current = await Location.watchPositionAsync({ accuracy: Location.Accuracy.BestForNavigation, distanceInterval: 5 }, (location) => {
      ...
      <View style={styles.content}>
        ...
        <View style={styles.controls}>
          <TouchableOpacity style={styles.btn} onPress={() => setIsActive(!isActive)}>
            {isActive ? (<Pause color="#ffaa00" size={40}/>) : (<Play color={CYBER_THEME.primary} size={40}/>)}
          </TouchableOpacity>

          <TouchableOpacity style={[styles.btn, { borderColor: "#ff0000" }]} onPress={triggerStopSequence}>
            <Square color="#ff0000" size={30} fill="#ff0000"/>
          </TouchableOpacity>
        </View>
      </View>

      <Modal visible={showModal} transparent animationType="slide" onRequestClose={() => !isSyncing && setShowModal(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>SAVE_ACTIVITY_LOG</Text>

            <View style={styles.mapContainer}>
              {coords.length > 0 ? (<MapView style={styles.map} provider={PROVIDER_GOOGLE} customMapStyle={mapStyle} initialRegion={{
                latitude: coords[0].latitude,
                longitude: coords[0].longitude,
                latitudeDelta: 0.01,
                longitudeDelta: 0.01,
            }} scrollEnabled={false} zoomEnabled={false}>
                  <Polyline coordinates={coords} strokeColor={CYBER_THEME.primary} strokeWidth={3}/>
            ...
            <TextInput style={styles.input} placeholder="SEQUENCE_TITLE" placeholderTextColor="#444" value={title} onChangeText={setTitle} editable={!isSyncing}/>
            ...
            <TouchableOpacity style={[styles.saveBtn, isSyncing && { opacity: 0.5 }]} onPress={handleSave} disabled={isSyncing}>
              <Text style={styles.saveBtnText}>
                {isSyncing ? "SYNCING_WITH_CORE..." : "UPLOAD_TO_CORE"}
              </Text>
            </TouchableOpacity>

            {!isSyncing && (<TouchableOpacity onPress={() => setShowModal(false)}>
