...
import { StyleSheet, ScrollView, TouchableOpacity, Image, View, Text, Dimensions, Animated, PanResponder, } from "react-native";
    ...
    const fetchStats = async () => {
        try {
            ...
            if (recent.length > 0) {
                ...
                setChartData({
                    labels: rawData.map((a) => new Date(a.createdAt).toLocaleDateString("en-US", {
                        weekday: "short",
                    })),
                    datasets: [
                        {
                            data: rawData.map((a) => parseFloat(a.distance)),
    ...
    return (<View style={{ flex: 1, backgroundColor: "#000" }}>
        ...
        <View style={styles.container}>
          ...
          <View style={styles.header}>
            ...
            <TouchableOpacity onPress={() => router.push("/profile")} style={styles.avatarContainer}>
              {user?.image ? (<Image source={{ uri: `${BASE_IMAGE_URL}${user.image}` }} style={styles.avatarCircle}/>) : (<View style={styles.avatarPlaceholder}>
          ...
          <View style={styles.chartCard}>
            ...
            <LineChart data={chartData} width={screenWidth - 70} // Adjusted for inner padding
     height={220} chartConfig={{
      ...
      <Animated.View style={[
        ...
        <TouchableOpacity style={styles.playCircle} activeOpacity={0.8} onPress={handleStartActivity}>
